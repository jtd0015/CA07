...
public class OrderBook {
	...
	public void addToOrderBook(Order order) {
		if (order.isOrderType() == 1){
			ArrayList<Order> tempOrders = sellOrders.get(order.getStockSymbol());
			tempOrders.add(order);
			sellOrders.put(order.getStockSymbol(), tempOrders);	
		}
		else if (order.isOrderType() == 2){
			ArrayList<Order> tempOrders = buyOrders.get(order.getStockSymbol());
			tempOrders.add(order);
			buyOrders.put(order.getStockSymbol(), tempOrders);
		}
	}

	public void trade() {
		HashMap<String, Stock> stockList = m.getStockList();
		Set<String> stockKeys = stockList.keySet();
		String[] stocks = stk.toArray(new String[stockKeys.size()]);
		...
		{		
			...
			if(supply != null && demand != null && supply.size() > 0 && demand.size() > 0)
			{
				...
				int supplyCount = 0;
				int demandCount = 0;
				while (supplyCount < supply.size())
				{
					Order temp = supply.get(supplyCount);
				...
				while (demandCount < demand.size())
				{
					Order temp = demand.get(demandCount);
	...
	private void sortByPrice(ArrayList<Order> orders)
	{
		...
		while (i < orders.size())
		{
			...
			while (j < orders.size() - 1)
			{
				...
				if (tempOrder.getPrice() < tempNext.getPrice())
				{
